...
class GameStoreImplUnitTest {
    ...
    void addNewTest() {
        ConcurrentLinkedQueue<Movement> movementQueue = new ConcurrentLinkedQueue<>();
        gameList.add(generateBaseGameWithUser().setMovementQueue(movementQueue));
        store.addNewMovement(TEST_USERNAME, Movement.MOVE_LEFT);
    ...
    @Test
    void findStartTimeTest() {
        gameList.add(generateBaseGameWithUser().setStartTime(LocalDateTime.MAX));
        Assertions.assertEquals(LocalDateTime.MAX, store.findStartTime(TEST_USERNAME));
    }
